...
sub extractReleaseNameInfo {
	...
	my $indexYear;
	if ($data = $findLast->($canonReleaseName, qr/(?:^|\D)(19[3-9]\d|20\d{2})(?:\D|$)/)) {
	...
	my $indexYmd;
	if ($data = $findLast->($canonReleaseName, qr/(?:^|\D)((?:19[3-9]\d|20\d{2})\s\d{1,2}\s\d{1,2})(?:\D|$)/)) {
		$indexYmd = $data->{index};
	...
	if ($isTvOrMovie) {
		...
		if ($out->{season} || $out->{episode} || ($out->{source} && $out->{source} =~ /HDTV|PDTV/i)) {
			if ($canonReleaseName !~ /(?:^|\D)(?:19[4-9]\d|20\d{2})\s+\d\d\s+\d\d(?:\D|$)/) {
				$yindex = undef;
			}
		}
